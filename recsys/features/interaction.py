...
def generate_interaction_data(trans_df):
    ...
    chunk_size = 1000
    for chunk_start in tqdm(range(0, len(unique_customers), chunk_size), desc="Processing customer chunks"):
        ...
        for customer_id in chunk_customers:
            ...
            if available_articles and num_ignores > 0:
                ignore_timestamps = generate_timestamps(last_purchase_timestamp, num_ignores, 1, 96)
                ...
                for ts, art_id in zip(ignore_timestamps, selected_ignores):
                    ...
                    for _ in range(num_ignore_events):
                        ignore_ts = ts - np.random.randint(1, 12) * 3600000  # Add some random hours difference
                        interactions.append({
                            't_dat': ignore_ts,
            ...
            for row in purchase_rows:
                ...
                if np.random.random() < CLICK_BEFORE_PURCHASE_PROB:
                    ...
                    for _ in range(num_pre_clicks):
                        click_timestamp = generate_timestamps(purchase_timestamp, 1, 1, 48)[0]
                        interactions.append({
            ...
            if np.random.random() < EXTRA_CLICKS_PROB:
                ...
                available_for_clicks = list(all_articles_set - 
                ...
                if available_for_clicks and num_extra_clicks > 0:
                    click_timestamps = generate_timestamps(last_purchase_timestamp, num_extra_clicks, 1, 72)
